...
export default function GamePage() {
  ...
  useEffect(() => {
    ...
    socket.on('grid_update', ({ grid, row, col, letter, placedBy, newWords, scores, currentTurn }) => {
  ...
  const gameEnded = useGameStore((s) => !s.gameStarted && s.scores.length > 0 && s.grid.length > 0);
      ...
      <div className="min-h-screen flex items-center justify-center p-6">
        ...
        >
          ...
          <div className="flex flex-col gap-2 w-full">
            {sorted.map((p, i) => (
              <div key={p.id} className={`flex justify-between px-4 py-3 rounded-xl ${i === 0 ? 'bg-amber-500/20 border border-amber-500/30' : 'bg-white/5'}`}>
                <span className="font-medium text-white">{i === 0 ? '🏆 ' : ''}{p.name}</span>
                <span className="font-mono font-bold text-violet-300">{p.score} pts</span>
              </div>
            ))}
          </div>
          <button className="btn-primary w-full" onClick={handleLeave}>Back to Home</button>
        </motion.div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex flex-col">
      {/* Top bar */}
      <header className="flex items-center justify-between px-6 py-4 border-b border-white/8 glass-card rounded-none">
        <div className="flex items-center gap-3">
          <span className="text-xl font-extrabold bg-gradient-to-r from-violet-400 to-pink-400 bg-clip-text text-transparent">
            WordGrid
          </span>
          <span className="glass-card-sm px-3 py-1 text-xs font-mono text-violet-300">{roomCode}</span>
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <Zap size={14} className={isMyTurn ? 'text-violet-400' : 'text-white/30'} />
      ...
      <AnimatePresence>
        {lastWordFlash && (
          ...
          >
            ...
            <span className="text-emerald-400 text-sm">+{lastWordFlash.word.length} pts</span>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <main className="flex-1 flex items-start justify-center gap-6 p-6 overflow-auto">
        ...
        <div className="flex-1 flex items-start justify-center min-w-0">
          <div className="flex flex-col items-center gap-4">
            {/* Grid size label */}
            <div className="text-white/30 text-xs font-mono">{gridSize} × {gridSize} grid</div>
            <Grid />
          </div>
        </div>
